...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        List<String> collectNoDist = transactions.stream()
                .map(transaction -> transaction.getTrader().getCity()) // 도시 이름 추출 후
                .distinct()  // 중복 제거
                .collect(toList());

        collectNoDist.forEach(System.out::println);
        ...
        List<Trader> cambridgeTrd = transactions.stream()
                // filter 먼저 적용 후 map을 하면 더 좋다!
                .map(transaction -> transaction.getTrader())
                .filter(c -> c.getCity().equals("Cambridge"))
                .sorted(comparing(trader -> trader.getName()))
                .collect(toList());

        cambridgeTrd.forEach(System.out::println);
        ...
        List<String> nameList = transactions.stream()
                ...
                .sorted()  // 여기서는 굳이 Comparator 안 써도 됨
                .collect(toList());
